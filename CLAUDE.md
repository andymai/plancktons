...
`ALGORITHM_VERSION` versions the **face-mating growth/study/CSV** kernel only. The vacuum-bag physics (below) is a separate browser-only system that never flows through the study/CSV pipeline, so it carries its own `VACUUM_ALGORITHM_VERSION` in `src/lib/vacuum.ts` — do **not** bump `ALGORITHM_VERSION` for vacuum-physics changes, and do not stamp vacuum results into the assembly JSON export.

### The vacuum-bag scene is a separate, browser-only physics system

`src/lib/vacuum.ts:runVacuumSettle` is a deterministic, frictionless 6-DOF rigid-body settle: N loose Plancktons are squeezed into a jammed packing by a contracting spherical "bag". It does **not** use `assembly.ts`/`matePlanckton` — tets are free-floating and only touch where the physics pushes them. Determinism comes entirely from the seeded LCG (`rng.ts`); there is **no** wall-clock or global RNG anywhere in `vacuum.ts`/`rigidTet.ts`/`quat.ts` (a guard test in `tests/vacuum.test.ts` greps for it), and a fixed timestep makes the same seed reproduce a byte-identical trajectory. Supporting pure modules: `quat.ts`, `mat3.ts`, `inertia.ts` (tet mass properties), `rigidTet.ts` (rigid body + frictionless normal-impulse contacts), `contactGraph.ts` (contact-coordination metric, the loose-packing analogue of `meanTetCoordination`), `vacuumMetrics.ts` (reuses `hull`/`morphology`/`voronoi`/`shape` so η is comparable to growth), and `marchingCubes.ts` (marching-_tetrahedra_ iso-surface for the wrinkled skin, fed by `morphology.ts:morphologicalField`). `planckton.ts:tetContact` is the MTV sibling of `tetsOverlap` used for contacts. The bag wall contracts only while max penetration stays below a tolerance (`PEN_TOL_FRAC`), applying real inward pressure to the bulk and jamming when further contraction can't be absorbed. The settle is precomputed off the main thread (worker `vacuum` job) and replayed via a scrub timeline; `VacuumScene.tsx` reconstructs each tet per frame from `(position, quaternion)` and `VacuumBagMesh.tsx` morphs the skin between a sphere (air-in) and the morphological hull (sealed). The worker pool/CLI are untouched — a settle is one job, not a sweep.
